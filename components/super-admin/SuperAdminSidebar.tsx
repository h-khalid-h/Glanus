...
export function SuperAdminSidebar() {
    ...
    return (
        <aside className="hidden lg:flex lg:flex-col fixed inset-y-0 left-0 z-50 w-64 border-r border-border/50 bg-[#07101f]">
            {/* Brand */}
            <Link
                href="/super-admin"
            ...
            <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
                {NAV.map(({ href, label, icon: Icon, exact }) => {
                    ...
                    return (
                        ...
                        >
                            <Icon className={`h-4 w-4 shrink-0 ${active ? 'text-indigo-400' : 'text-muted-foreground/60 group-hover:text-muted-foreground'}`} />
                            {label}
            ...
            <div className="shrink-0 border-t border-border/50 p-3 space-y-1">
                <Link
                    href="/workspaces"
                    className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs text-muted-foreground transition-colors hover:text-foreground hover:bg-muted/40"
                >
