 ...
 *
 * Returns a paginated, searchable list of ALL workspaces enriched with
 * user count, asset count, agent count, and last activity timestamp.
...
export const GET = withErrorHandler(async (request: NextRequest) => {
    ...
    return runWithUserRLS(user, async () => {
        const result = await SuperAdminService.getWorkspaceList(page, limit, search);
        return apiSuccess({
