...
function WorkspaceDetailModal({ ws, onClose }: { ws: WorkspaceRow; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-lg rounded-2xl border border-border bg-surface-1 shadow-2xl shadow-black/60">
                {/* Header */}
                <div className="flex items-center justify-between border-b border-border px-6 py-4">
                    <div>
                        <h2 className="text-base font-semibold text-foreground">{ws.name}</h2>
                        <p className="text-xs text-muted-foreground font-mono mt-0.5">{ws.id}</p>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
                ...
                <div className="px-6 py-4 grid grid-cols-2 gap-x-6 gap-y-3">
                    {fields.map(([label, value]) => (
                        <div key={label}>
                            ...
                            <p className="mt-0.5 text-sm text-foreground font-mono truncate">
                ...
                <div className="flex items-center justify-end gap-2 border-t border-border px-6 py-4">
                    <a
                        ...
                        className="flex items-center gap-1.5 rounded-xl border border-indigo-500/30 bg-indigo-500/10 px-3 py-1.5 text-xs font-medium text-indigo-400 transition-colors hover:bg-indigo-500/20"
                    >
                    ...
                    <button
                        type="button"
                        onClick={onClose}
                        className="rounded-xl border border-border bg-muted px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:text-foreground"
                    >
                        Close
                    </button>
                </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">Browse, search, and inspect all tenant workspaces</p>
            </div>
